...
class ContextMenu extends React.Component {
    ...
    return (
      <React.Fragment>
        {_.map(blockTypeList, v => {
          if (v.value === 'barcode' || v.value === 'counter') return null
          return (
            v.value === 'image'
  ...
  renderOrderActionBtn = () => {
    ...
    const arr = name.split('.')
    const { dataKey } = editStore.config.contents[arr[2]]
    ...
    return (
      <React.Fragment>
        <div onClick={this.handleChangeTableDataKey.bind(this, 'quantity')}
          className={isQuantityActive ? 'active' : ''}>{i18next.t('入库数小计')}
        </div>
        <div onClick={this.handleChangeTableDataKey.bind(this, 'money')}
