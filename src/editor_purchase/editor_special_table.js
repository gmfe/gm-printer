...
class TableDetailEditor extends React.Component {
  ...
  render () {
    const { addFields: { detailFields } } = this.props
    const { dataKey, specialConfig: { template_text, style } } = this.props.config
      ...
      <div>
        ...
        {dataKey !== 'purchase_no_detail' && <React.Fragment>
          ...
          <div className='gm-padding-top-5'>
            ...
            <Separator/>
            <TextAlign style={style} onChange={this.handleSpecialStyleChange}/>
            <Gap/>
            <Textarea onChange={this.handleSpecialTextChange} value={template_text} placeholder={i18next.t('请输入明细字段')}/>
          </div>
  ...
  render () {
    ...
    if (editStore.computedRegionIsTable) {
      ...
      if (tableConfig.specialConfig) {
        return <TableDetailEditor config={tableConfig} addFields={this.props.addFields}/>
      } else {
        return null
      }
    }
