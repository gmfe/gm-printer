...
function generateTable (tasks) {
  return tasks.map((task, index) => {
    ...
    return {
      ...
      [i18next.t('单价_基本单位')]: `${money(task.purchase_price || 0)}${Price.getUnit() + '/'}${task.std_unit_name}`,
      [i18next.t('单价_采购单位')]: `${money(Big(task.purchase_price || 0).times(task.ratio))}${Price.getUnit() + '/'}${task.purchase_unit_name}`,

      [i18next.t('一级分类')]: task.category_name_1,
      [i18next.t('二级分类')]: task.category_name_2,
      [i18next.t('品类')]: task.pinlei_name,

      [i18next.t('参考成本')]: `${task.ref_price ? money(task.ref_price) : 0}${Price.getUnit() + '/'}${task.std_unit_name}`,
      [i18next.t('规格')]: `${task.ratio}${task.std_unit_name}/${task.purchase_unit_name}`,
      ...
      [i18next.t('采购金额')]: money(Big(task.purchase_price || 0).times(task.already_purchased_amount)),
      ...
      __details: task.address.map(o => {
        return {
          ...
          [i18next.t('采购数量_采购单位')]: Big(o.plan_amount).div(task.ratio).toFixed(2),
...
function purchaseBill (data) {
  ...
  _.each(tasks, task => {
    if (task.plan_amount) {
      plan_money = Big(task.plan_amount).times(task.purchase_price).plus(plan_money)
    }
    if (task.already_purchased_amount) {
      pur_money = Big(task.already_purchased_amount).times(task.purchase_price).plus(pur_money)
    }
  })
