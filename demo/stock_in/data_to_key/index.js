...
const getTableData = (data, type) => {
  const ordinary = _.map(data.details, (item, index) => {
    const purchase_unit_quantity = Big(item.quantity || 0).div(item.ratio).toFixed(2)
    return {
      ...
      [i18next.t('入库单价_基本单位')]: item.unit_price !== null ? Big(item.unit_price).toFixed(2) : '',
      [i18next.t('入库金额')]: item.money !== null ? Big(item.money).toFixed(2) : '',
      [i18next.t('商品备注')]: item.remark,
      [i18next.t('商品ID')]: item.spu_id,
      [i18next.t('最高入库单价')]: item.max_stock_unit_price !== null ? Big(item.max_stock_unit_price).toFixed(2) : '',
      [i18next.t('入库数_包装单位')]: purchase_unit_quantity,
      [i18next.t('入库单价_包装单位')]: purchase_unit_quantity === 0 ? '' : Big(item.money || 0).div(purchase_unit_quantity).toFixed(2),
      [i18next.t('操作人')]: item.operator,
      [i18next.t('存放货位')]: item.shelf_name,
      [i18next.t('保质期')]: item.life_time,
      [i18next.t('生产日期')]: item.production_time,
      [i18next.t('补差金额')]: item.different_price !== null ? Big(item.different_price).toFixed(2) : ''
    }
  })
  const sumMoney = Big(_.reduce(data.details, (a, b) => a + parseFloat(b.money), 0)).toFixed(2)
...
const generateUpperPrice = (data) => {
  return {
    ...
    [i18next.t('整单金额_大写')]: coverDigit2Uppercase(Big(data.delta_money || 0).plus(data.sku_money || 0).toFixed(2))
  }
}
...
const formatData = (data) => {
  return {
    common: {
      ...
      [i18next.t('整单金额')]: Big(data.delta_money || 0).plus(data.sku_money || 0).toFixed(2),
